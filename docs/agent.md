...
A workspace that lists more than one agent shows an **Agent** row too
(claude, codex); `{agent}` on the route opens `corgi agent codex` instead
(2.30.17).
...
workspace that does not list it, is a 400 with the reason.
