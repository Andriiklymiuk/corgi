...
| `corgi agent today [--since 8h] [--write] [--json]` | what has been done today since midnight: commits, prompts, the watch's unattended runs and their pull requests; `--json` adds `waits` (how often sessions waited on you) and `days` (a fortnight of sessions, messages and tool calls per day, every account summed) — the numbers the phone's share card draws (2.22.6) |
