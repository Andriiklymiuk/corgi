...
| `corgi agent digest [--send]` | today's one-message summary; with `digestAt: "20:00"` in the agent config the daemon sends it once a day where notifications go — and, from 2.22, to the phone as a push that opens the brief (`/launch/brief`: the digest plus the standup since yesterday) |
