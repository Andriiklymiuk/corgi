...
Check whether agent mode can actually work here

```
corgi agent doctor [flags]
```

### Options

```
      --away       also what matters when nobody touches this machine for weeks: updates, lid, power, network, heat, pulse, digest, isolation
  -h, --help       help for doctor
      --security   only the security checks: deny rules, the secrets hook, permissions, isolation, tunnel exposure
```
