# corgi agent scope add

## corgi agent scope add

Widen a ticket's scope by one path, on the record

```
corgi agent scope add <ref> --path <glob> [flags]
```
...
```
  ...
  -h, --help               help for add
      --path stringArray   a glob to add (repeatable)
```
