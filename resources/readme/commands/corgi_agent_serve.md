...
Supervise Remote Control for every enabled workspace

```
corgi agent serve [flags]
```

### Options

```
      --foreground   Run in this terminal and mirror the supervised process's output. Off by default: that output can contain env values and tokens, and in serve mode corgi's stderr is a log file.
