...
| `corgi_http` | `service, path, method?, body?, headers?` | `{status, headers, body, truncated, ms}` | one request to a running service by name on 127.0.0.1; JSON body sent as JSON; body capped at 64 KB; **tunnel-gated**. The way to check a route as a client would |
