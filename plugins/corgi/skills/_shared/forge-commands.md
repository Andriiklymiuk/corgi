# GitHub / GitLab commands (gh / glab)
...
## 6. Open a draft PR/MR, flip ready, watch CI, merge

Run `gh`/`glab` from inside the repo dir (they read the repo from cwd; `git -C` only
sets git's dir, and `--body-file` paths are relative to cwd).
...
gh pr comment <n> --body-file docs/stories/<issue-key>-<slug>.md   # spec on the PR
gh pr edit <n> --body-file <tmp>                                    # rewrite the body (risk card)
gh pr checks <n> --watch --fail-fast                                # or poll --json name,bucket
gh pr view <n> --json state,isDraft,mergedAt                        # merged? in flight?
gh pr ready <n>            # flip ready (undo: gh pr ready <n> --undo); only when a human says so
gh pr merge <n>            # only when a human says so
...
glab mr note create <iid> -m "$(cat docs/stories/<issue-key>-<slug>.md)"   # spec on the MR
glab mr update <iid> --description "$(cat <tmp>)"                          # rewrite the body
glab ci status --branch <branch>   # or glab ci view; poll glab mr view <iid> until not running/pending
