...
- **Risk ceiling** (config `maxRisk`, default 6): `stories` stamps every draft PR with the `risk` skill's card and prints `risk N/10 <tier>` per story. A story whose score is above `maxRisk` is still opened as a draft — the work is not thrown away — but the heartbeat's `--note` carries `needs human review: <key> risk N/10 <tier>` and the iteration's printed line leads with it. The pre-build **forecast** (`stories` Phase 1) applies the same ceiling earlier: a story forecast above it is staged for the human at the spec gate with the forecast attached, never built unattended. Nothing here auto-merges at any score; `auto-approve: yes` on a card is information for the human who flips the draft, not an instruction to this loop.
