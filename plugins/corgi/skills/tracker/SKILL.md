...
- **Plan, don't build** — code → `stories`, ideas → `suggest`. **Never touch
  `manualRun`.**
- **No AI attribution.** Everything posted or committed — ticket comments, PR/MR
  bodies, commit messages, store listings/notes — reads as if a human wrote it. Never
  add "generated by AI / Claude / corgi", a co-authored-by or AI trailer, a bot
  signature, or a robot-emoji badge to any outward text.
