...
- **No AI attribution anywhere.** Ticket comments, PR/MR bodies, commit messages all
  read as human-written. Never add "generated by AI / Claude / corgi", a co-authored-by /
  AI trailer, a bot signature, or a robot-emoji badge.
...
- **Watch CI to green (when asked, or in a pre-authorized autonomous run).** After the
  ...
  "opened".
  - **GitHub:** `gh pr checks <n> --watch --fail-fast` (or poll `gh pr checks <n> --json
    name,bucket` until none are `pending`).
  - **GitLab:** `glab ci status --branch <branch>` / `glab ci view` (or poll
    `glab mr view <iid>`) until the pipeline leaves `running`/`pending`.
