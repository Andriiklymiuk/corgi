...
func serveMCPHTTP(s *server.MCPServer, addr, token string, opts mcpHTTPOpts) {
	...
	if token != "" || deviceStore != "" {
		...
		mux.Handle("/launch/interrupt", bearerAuth(token, http.HandlerFunc(launchInterruptHandler), deviceStore))
		...
		mux.Handle("/launch/refresh", bearerAuth(token, http.HandlerFunc(launchRefreshHandler), deviceStore))
		mux.Handle("/launch/push", bearerAuth(token, http.HandlerFunc(launchPushHandler), deviceStore))
		mux.Handle("/launch/work-on", bearerAuth(token, http.HandlerFunc(launchWorkOnHandler), deviceStore))
		mux.Handle("/launch/task", bearerAuth(token, http.HandlerFunc(launchTaskHandler), deviceStore))
